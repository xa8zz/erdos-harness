...
Fix $A_0>2$ and set
\[
  k:=\left\lfloor\frac{n}{2A_0\log n}\right\rfloor.
\]
Shortener plays the smallest legal odd prime for her first $k$ turns.  The
same Chebyshev log-mass induction, now with $j\le n/(2A_0\log n)$, gives
\[
  q_j\le A_0j\log n
  \qquad (1\le j\le k).
\]
Indeed, if all odd primes below $A_0j\log n$ were blocked, Prolonger's first
...
$A_0j\log n$ because $A_0>2$.

It follows that
\[
  \sum_{j=1}^k\frac1{q_j}\ge \frac1{A_0}-o(1).
\]
...
\[
  s_t\ge \frac1{A_0}-o(1).
\]
Since $1/q_j\le1/3$ and $A_0>2$, the partial sum remains in $[0,1]$.  The
second-order Bonferroni estimate gives
\[
  \L(n)
  \le
  \frac n2
  \left(1-\frac1{A_0}+\frac{1}{2A_0^2}\right)
  +o(n).
\]
Letting $A_0\downarrow2$ yields
\[
