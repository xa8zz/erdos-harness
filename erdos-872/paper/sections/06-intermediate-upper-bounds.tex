...
\]
uniformly for $j\le k$.  For this fixed $\varepsilon$,
\[
  \sum_{j=1}^k\frac1{q_j}\ge\frac{1}{3+2\varepsilon}-o_\varepsilon(1).
\]
Since the strategy and the preceding estimate hold for every
$\varepsilon>0$, we may let $\varepsilon\downarrow0$ after taking
$n\to\infty$, and therefore
\[
...
\]
Shortener plays the smallest legal odd prime for her first $k$ turns.
(Legality of such a prefix for all sufficiently large $n$ is proved in
\Cref{lem:prefix-existence}; the argument there depends only on board size, not
on the odd-part compression used in this section.)  The
...
\[
  ...
  +o(n).
\]
For every fixed $A_\star>2$ the strategy above is valid.  Letting
$A_\star\downarrow2$ after taking $n\to\infty$ yields
\[
