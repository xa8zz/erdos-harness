...
must not be used.  The obstruction is concrete: the top flat block contributes
up to $K \le n/(2\log n)$ indices with $b_j = n$, yet every window
$[n,(1+o(1))n]$ contains only $o(n/\log n)$ odd primes, so no uniform
$p_j/b_j = 1 + o(1)$ can supply the top block.  The correct bridge is the
half-density construction of \Cref{thm:prime-bridge}, which assigns the top
flat block separately to $[n,2n]$---whose $(1+o(1))n/\log n$ odd primes
exceed the demand $K$ by a factor of two---and uses queued coarse-bin
matching on the genuine blocks, where the envelope bound
$\tfrac{1}{(h+1)\log X}\le\tfrac{1}{2\log X}$ on the local $b$-density
supplies slack against the odd-prime density $1/\log X$.
...
\begin{proof}
Throughout this proof $A=A_h=n^{\alpha_h}\to\infty$ while $s=s(a)$ is fixed,
so
\[
  \log(Aa^j)=\log A+j\log a=(1+o_{H,a}(1))\log A
\]
uniformly for $0\le j\le s+1$.  The resulting $o(1)$ multiplicative errors in
the denominators below are absorbed into the $(1\pm\eta)$ margins, all valid
for all large $n$.
