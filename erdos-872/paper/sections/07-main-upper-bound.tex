...
\begin{proof}
Choose a sequence $H_m\to\infty$ such that
\[
  |J_r^{(H_m)}-J_r|\le\frac1m
  \qquad (1\le r\le4).
\]
For each $m$, choose $\lambda_m>1$ so close to $1$ that the
$O_{H_m,r}(\lambda_m-1)$ term in \Cref{prop:queued-prime-rounding} is at most
$1/m$ for $1\le r\le4$, and choose $a_m>1$ with $a_m^2\le\lambda_m$.  For
fixed $m$, enlarge $N_m$ so that, for all $n\ge N_m$, the construction of
\Cref{prop:envelope-inversion} with $H=H_m$ is valid,
\[
  |T_r^{(b)}(n)-J_r^{(H_m)}|\le\frac1m,
  \qquad
  |T_r^{(p)}(n)-T_r^{(b)}(n)|\le\frac2m
\]
for $1\le r\le4$, where the second inequality uses
\Cref{prop:queued-prime-rounding}.  We may take the $N_m$ increasing.  For
$N_m\le n<N_{m+1}$ use the parameters $H=H_m$, $\lambda=\lambda_m$, and
$a=a_m$.  Then $H_m\to\infty$ and $\lambda_m\downarrow1$, so the displayed
bounds give
\[
  T_r^{(b^{(n)})}(n)=J_r+o(1),\qquad
  T_r^{(p)}(n)=T_r^{(b^{(n)})}(n)+o(1)=J_r+o(1)
\]
for every fixed $1\le r\le4$.  The inequality $q_j\le p_j$ follows from
