...
\[
  n/\lambda^r < b_I\le n.
\]
We control this boundary layer by a cell count.  Fix a constant $c>1$, and
partition the positive axis into cells $[c^\ell,c^{\ell+1})$.  By
\Cref{lem:cell-reciprocal-mass}, every cell has $b$-reciprocal mass
$O_{H,c}(1/\log n)$.  There are $O_{H,c}(\log n)$ cells meeting the range of
the $b_j$.  For the product strip
\[
  n/\lambda^r < x_1\cdots x_r\le \lambda^r n,
\]
once the first $r-1$ cell indices are chosen, only
$O_{\lambda,c,r}(1)$ choices of the last cell can meet the strip.  Thus there
are $O_{H,\lambda,c,r}((\log n)^{r-1})$ relevant $r$-cell patterns.  Each
pattern contributes at most $O_{H,c,r}((\log n)^{-r})$ reciprocal mass, so the
total contribution of the strip is
\[
  O_{H,\lambda,c,r}\!\left(\frac1{\log n}\right)
  =
  o_{H,\lambda}(1).
\]
In particular the narrower boundary $n/\lambda^r<b_I\le n$ contributes
$o_{H,\lambda}(1)$.  Thus
\[
  T_r^{(p)}(n)
  =
