...
The greedy rule, together with the fact that we process prime bins in
increasing order and assign the smallest available prime within each bin to
the oldest unassigned demand, ensures the assigned prime sequence
$p_1<p_2<\cdots<p_K$ is strictly increasing.
...
Choose $N_m$ recursively increasing so that for every $n\ge N_m$ the following
ten conditions hold:
\begin{enumerate}
  ...
  holds for $C=\lambda_m^r$ and $1\le r\le4$, with the associated finite
  ...
  \[
    \sum_{j\in E}\frac1{b_j}\le \frac{C_m}{\log n},
  \]
  for the fixed constant $C_m$ attached to $H_m,a_m,s_m$;
