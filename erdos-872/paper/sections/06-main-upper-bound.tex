...
\begin{enumerate}
  ...
  \item a prime-rounding bridge that replaces the real sequence $b_j$ by actual
    odd primes $p_j$, using the key half-density slack from R60.
\end{enumerate}
...
The audit of R57 identified that the comparison theorem must be stated for
\emph{prime sequences}, not for arbitrary reals.  The repaired statement is the
following.
...
must not be used.  The correct repair is the R60 half-density bridge.
