...
\begin{theorem}[T2 lower bound]\label{thm:intro-t2}
...
\end{center}
The lower and upper bounds in this paper show that the answer, whichever it is,
lies between $n(\log\log n)^2/\log n$ and $0.19n$.

\subsection{Organization}

In \Cref{sec:shield-reduction} we prove the shield reduction theorem.  Section
\ref{sec:lower-bounds} contains T1 and T2.  In
\Cref{sec:524-cover,sec:intermediate-upper} we record the exact $5/24$ cover
theorem and the intermediate upper bounds $13/36$ and $5/16$.  Section
\ref{sec:main-upper} proves the main upper bound below $0.19n$, incorporating
the R60 repair of the prime-rounding bridge.  Section
\ref{sec:restricted-class} treats the two restricted carrier classes.
\Cref{sec:proof-class-barriers,sec:structural-negatives} collect the positive
packet dichotomy and the structural obstruction results.  Section
\ref{sec:formal-verification} summarizes the Lean and Aristotle artifacts, and
the appendices record formalization metadata, numerical checks, and glossary
information.
