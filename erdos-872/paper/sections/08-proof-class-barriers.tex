\section{Proof-Class Barrier Taxonomy}\label{sec:proof-class-barriers}
...
particular, the headline upper bound of \Cref{thm:main-upper} is proved
...
\begin{enumerate}
  \item \emph{Strategy-independent} results: Shield Reduction, Theorem~A, the
  conditional T2 construction subject to \Cref{prop:t2-finite-capture}, the
  Round~15 $\sigma_{15}$ upper bound, the separator theorems of
...
\Cref{thm:main-upper} is independent of $\sigma^*$.
\end{proposition}
...
\subsection{Conditional $\sigma^*$ Results}

We record the most useful $\sigma^*$-dependent statements for completeness, but
we emphasize that none of them is used in \Cref{sec:main-upper}.  Their
...
\Cref{sec:main-upper} is proved for $\sigma_{15}$ and does not use any of
