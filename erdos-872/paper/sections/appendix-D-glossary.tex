...
\begin{description}
  ...
  \item[$\U$] The upper half $(n/2,n]\cap \Z$ of the board.  Any subset
  ...
  \item[$\beta(P)$] The maximum total weight $\sum_{x\in B}w_n(x)$ over antichains
  $B\subseteq L(P)$ in the lower half; the largest weighted lower-half antichain
  surviving after Prolonger plays $P$.
\end{description}
...
\begin{description}
  ...
  \item[T2] The conditional Maker-first hypergraph-capture lower bound at
  scale $n(\log\log n)^2/\log n$, subject to the finite safe-edge hypothesis
  of \Cref{prop:t2-finite-capture}.
...
\begin{description}
  ...
  $K=\lfloor (1-\eps)n/(2\log n)\rfloor$ turns.
