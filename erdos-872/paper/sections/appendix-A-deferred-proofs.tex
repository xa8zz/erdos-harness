...
\begin{proposition}[Disjoint small-prime carriers]\label{prop:disjoint-carriers}
...
\begin{proof}
Let $B$ be the set of primes which occur in composite Prolonger moves.  By the
disjoint-support hypothesis, each $p\in B$ belongs to a unique composite
carrier $C(p)$.

Shortener uses the following priority list, always skipping moves which have
already become illegal.

First, she plays every legal prime.  This resolves every prime not belonging
to a composite Prolonger carrier, and also every prime which Prolonger has not
already made illegal by placing it inside a carrier.
