---
id: R177-repaired-manuscript
type: synthesis
date: 2026-07-23
intent: >
  Produce a self-contained final manuscript for the divisor-antichain game
  after an independent adversarial referee found only formal and scheduling
  defects that do not alter the numerical argument.
predecessors: [R176-packet-packing-preemption, V176-referee-audit]
action:
  kind: supersedes
  target: R176-packet-packing-preemption
claim: >
  The robust-game proof yields L(n)=o(n) after explicitly defining the finite
  extensive-form game with value inf over Shortener strategies of sup over
  adaptive Prolonger and erasure strategies, bounding each cone's local
  opening by max(b+1,2)=b+1, and completing Shortener's policy on positions
  containing only exceptional legal vertices.
failure_mechanism: >
  The superseded manuscript left the robust-game strategy quantifiers and
  erasure timing implicit, incorrectly said that only one post-opening
  Prolonger selection could precede a newly activated cone's first local
  Shortener response, and omitted a Shortener move when no root or active
  local move was available but a legal exceptional vertex remained.
confidence_at_time: high
strategy_dependence: independent
prompt: erdos-872/prompts/followup-R177-repaired-manuscript.md
---
...
4. If Shortener selects inside \(C_t\) after a global Prolonger selection or pass outside \(C_t\), insert a local Prolonger pass before that Shortener selection.
