---
id: R176-packet-packing-preemption
type: research
date: 2026-07-23
intent: >
  Follow-up pressing the packet dichotomy (packing vs pre-emption vs
  survivor-stock regularity) after the multipartite packet theorem killed
  wave-amortization arguments.
predecessors: [R175-piercing-race-inflation]
action:
  kind: extends
  target: R175-piercing-race-inflation
claim: >
  CLAIMED FULL PROOF of L(n) = o(n). Mechanism: (1) a robust envelope game
  V_b(N) on divisibility downsets of [N] (Prolonger gets b opening moves,
  may pass, and has an allied adversary erasing arbitrary upsets for free)
  dominating the true game, L(n) <= V_1(n); (2) the factorization x =
  a_K(x) t_K(x) into a K-dense block and a tag with P^-(t) > K a; all but
  an explicitly bounded exceptional set of integers in (delta N, N] have
  tags satisfying the self-roughness P^-(t) > H N/t with H = K delta > 1
  (Selberg sieve bound Phi(X,z) << X/log z; K-dense integers have density
  << X/(log X)^kappa, kappa = c0 log c0 - c0 + 1, c0 = 1/log 2); (3) Lemma
  3.1: two self-rough tags with lcm <= N are nested, so minimal (root)
  tags generate pairwise-disjoint, mutually incomparable cones covering
  all but the exceptional set; external selections project into a cone as
  principal quotient upsets, making play inside a cone exactly the robust
  game on a downset of [q_t] with one extra opening move; (4) Shortener's
  root-cone sweep (greedily fire the largest live unprocessed root tag)
  pairs every Prolonger cone-activation after the first with an earlier
  fired cone of at least equal weight, giving activated weight <= half of
  total weight + (b+1) N^gamma/delta; (5) dividing by N and taking N then
  A to infinity (delta = A^-2, K = ceil(e^{A^3})) yields the density
  recursion c_b <= (1/2) c_{b+1}; with the uniform cap c_b <= 1 the
  telescope forces c_1 = 0, hence V_1(N) = o(N) and L(n) = o(n). The
  packet obstruction is dissolved by localization: all useful divisors of
  a single vehicle lie in ONE root cone (their root tags would otherwise
  have a common multiple <= N), so packets cost activations, and length is
  counted in selections, never in deletion mass. Interior selections are
  bounded by the same count.
implications:
  - If verified, Erdos 872 is resolved: L(n) = o(n) unconditionally.
  - The 176-round kill-currency framing was the wrong coordinate system; selection-count localized to laminar self-rough cones is the right one.
  - The renormalization program (F14 scale induction) closes via the robust-game envelope: the erasure ally makes the game class closed under cone quotients, and the +1 opening absorbs all boundary error.
verifiers_at_time:
  - [fable-linewise-adversarial-read, cannot-break]
  - [fable-machine-check-decomposition-2.3, agrees]
  - [fable-machine-check-lemma-3.1-exhaustive-N1000, agrees]
  - [fable-machine-check-cone-disjointness, agrees]
confidence_at_time: medium
strategy_dependence: independent
prompt: erdos-872/prompts/followup-R176-packet-packing-preemption.md
---

I’ll seek a trajectory-wide, all-scales charging invariant for packet installation and pre-emption, testing whether it yields either linear packing or sublinear top survivors and interior moves.

A promising refinement tags each move by its entire rough part above a fixed power threshold, leaving only a Dickman-small smooth remainder; exact recursive scheduling remains to prove.

A new obstruction is that packet roots themselves kill board vertices, so lost-weapon charging is insufficient. I’m testing a global kill-per-pair invariant against the sharp star configuration.
