...
>
> **Plain resilience lemma REFUTED; scale-entropy form required (Pro R32).** The naive state inequality $|\operatorname{Cl}_h| \le O(\log h \cdot \mathfrak B_h + \text{scored})$ is FALSE. Simplex-star cascade counterexample: $X = \{0\} \cup Y$ with $|Y| = M$; scored star $\mathcal F = \{\{0\} \cup D : D \in \binom{Y}{h-1}\}$; residual $R = \binom{Y}{h}$ has $R \subseteq \operatorname{Cl}_h$, $\mathfrak B_h = 0$, and $|\operatorname{Cl}_h|/|\mathcal F| = (M-h+1)/h$ arbitrarily large. Embeds into divisibility geometry via short prime intervals $[P, (1+\eta)P]$. **But does NOT refute $L(n) = o(n)$**: the dangerous cascade has exponentially small reciprocal mass in its narrow prime cell. Corrected target: scale-entropy-weighted form with concentration term $\sum_\mathcal{Q} (n/\log n) \alpha(\mathcal Q)^h/h! = o((\log h/h) N_h)$; proof requires scale-local arithmetic robust transversal theorem handling multi-cell product stars. See researcher-32-pro-A-simplex-star-refutation.md.
>
> **Empirical Shortener strategy extracted (Codex R32).** Rule $\sigma$: "claim vertex of max unresolved-edge degree; tiebreak on initial degree then lexicographic." Matches 95.6% of exact-minimax Shortener states; empirical $g \approx 1 + O(1/h)$ on tested grids. Simplex-star cascade Prolonger NOT in Codex's test suite — $\sigma$-vs-cascade performance is open. See researcher-32-codex-sigma-rule-summary.md.
>
