Don't search the internet. This is a test to see how well you can craft non-trivial, novel and creative proofs given a "primitive sets and divisibility games" math problem. Provide a full proof or disproof of the finite-shell clearing statement (FSC) defined in the Problem section.

## Problem

For \(n\ge2\), two players alternately choose previously unchosen integers from \(\{2,\ldots,n\}\), always keeping the chosen set an antichain under divisibility (a primitive set). The game ends when the set is maximal. Prolonger moves first and maximizes the total number of moves; Shortener minimizes it; \(L(n)\) is the length under optimal play. The Attachment below defines, for fixed \(J\ge1\) and \(\eta>0\), the finite-shell clearing statement \(\mathrm{FSC}(J,\eta)\): after the accepted trapping prelude, Shortener processes the top \(J\) dyadic shells \(S_j=(n/2^{j+1},n/2^j]\) in order, playing while \(|Q_j(A)|>\eta|S_j|\) a currently legal lower element maximizing the number of deleted members of \(Q_j(A)\) (the live nonminimal shell population), every turn counted, no guards or optional activations; \(\mathrm{FSC}(J,\eta)\) asserts these \(J\) phases finish in \(o(n)\) Shortener turns against every Prolonger strategy. Prove or disprove \(\mathrm{FSC}(J,\eta)\), either for all fixed pairs or for one explicit fixed pair (state which).

Why this is decisive. The Attachment proves: if \(\mathrm{FSC}(J,\eta)\) holds for every fixed \(J\) and \(\eta>0\), then \(L(n)=o(n)\) — deep shells (\(j\ge J\)) are controlled by their TOTAL SIZE \(n/2^J\), not by clearing, so no growing-depth or coupled-modulus estimate is needed; this makes the family the weakest known sufficient target, immune by construction to terminal, activation, and parameter collapses. A disproof of even ONE fixed pair \((J,\eta)\) — a Prolonger strategy forcing the prescribed greedy phases to last \(\Omega(n)\) turns — closes this route decisively (note it would not by itself prove \(L(n)=\Omega(n)\): the policy is specific, and other policies remain; say precisely what your construction does and does not show). Formulation caution: if \(\mathrm{FSC}\) as stated admits a degenerate witness or hidden collapse (eighteen collapse families are catalogued in the record), proving THAT with the exact repair is a valuable resolution.
...
REMEMBER - this argument may require non-trivial, creative and novel elements; both directions of (FSC) are valuable, and the proof must handle arbitrary replies.

## Attachment (verbatim): the analysis defining FSC and proving its sufficiency
