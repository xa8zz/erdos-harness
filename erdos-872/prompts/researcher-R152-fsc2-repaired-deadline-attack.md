Don't search the internet. This is a test to see how well you can craft non-trivial, novel and creative proofs given a "primitive sets and divisibility games" math problem. Provide a full proof or disproof of the repaired second-shell clearing statement FSC(2, eta), stated exactly in Attachment E section 2 ("Exact repair") — with the activation data fixed in advance and 0 < eta_0 < eta/2.
...
Attachment E states the target exactly (its section 2): with the prelude, top-phase stopping rule at a fixed 0<eta_0<eta/2, and prefix-measurable tie-break all fixed before Prolonger is quantified, and prelude+top phase terminating in o(n) actual moves uniformly, there must not exist c>0, n_k->infty, H(n_k)->infty, and on-trajectory post-top times b_k in the active second-shell phase with |A_{b_k}|=o(n_k), |Q_1(A_{b_k})|>eta|S_1|, Delta_{Q_1}(A_{b_k})<=H(n_k), followed by at least c*n_k further Shortener turns with the phase active and degree at most H(n_k). Prove this for every fixed 0<eta<1, or disprove it for one explicit eta by constructing the Prolonger strategy realizing the capped tail on the required trajectory.

Why this is decisive. A proof proves FSC(2,eta) and is the first extension of shell clearing past the top shell (state whether your method iterates to deeper shells, which inherit blockers from every previously processed shell). A disproof closes the weakest known sufficient route via a concrete direct-ancestor mechanism (state exactly what it does and does not show about other Shortener policies and about L(n) itself). Formulation caution: if the repaired statement still admits a degenerate witness or hidden collapse (nineteen families are catalogued in the record), proving THAT with the exact repair is a valuable resolution.
...
- Second-shell monotone cap-entry normal form: along the second-shell greedy phase the maximum deletion degree is nonincreasing and its sum over turns is at most \(|S_1|\); every failure reaches, within \(o(n)\) turns, an active sparse capped-degree position and then sustains a linear capped tail — Attachment C (Lemma 1, Theorem 2).
- Doubling/direct-ancestor lemma: after the top shell is processed to \(|Q_0|\le\eta_0|S_0|\) with \(\eta_0=\eta/4\), while \(|Q_1|>\eta|S_1|\) at least \((\eta/8+o(1))n\) elements \(x\in Q_1\) have \(2x\) illegal, each certified by a previously selected \(a_x=2d_x\), \(d_x\mid x\), \(x/d_x\) odd; one blocker \(a=2d\) can certify \(\Theta(n/d)\) targets at once, so no per-blocker \(C\cdot\tau(a)\) capacity bound exists — Attachment C (Lemma 3) and Attachment D (Lemma 1).
- Coatom ownership / cumulative sparsity: a minimal legal \(x\) needs \(\omega(x)\) distinct blockers (one per coatom \(x/p\)); hence along any prefix of \(t=o(n)\) actual moves only \(o(n)\) integers of any fixed interval \((\alpha n,\beta n]\) EVER become minimal legal — Attachment D (Lemma 2, Corollary 3).
- Root reduction and deadlines (new, load-bearing): legal divisors of a legal target form an upset; every prescribed maximum-degree move has the same deletion set as a divisibility-minimal legal root, which it permanently kills; hence a \(T\)-turn second-shell phase generates \(\ge T\) distinct minimal-root events, a linear tail forces \(\ge cn\) of them to first become minimal AFTER sparse cap entry, and \(\ge(c/2)n\) of the charged roots lie in \(((c/2)n, n/4]\); every linear tail contains a linear suffix at a CONSTANT degree cap \(D(c)\) beginning after a fixed positive fraction of \(n\) turns; and any divisor \(d\) legal through a \(cn\)-turn tail has final-survivor incidence \(k_X(d)\le 1/(4c)+o(1)\) — Attachment E (Lemmas 1-2, Corollary 3, sections 4-6). The isolated open mechanism: whether one blocker move, divisible by many currently-legal roots at once, can satisfy many final-survivor deadlines simultaneously.
- Second-shell chronology-sensitive degree lemma (PROVEN): for primitive \(A\), \(|A|=t=o(n)\), legal \(Q\subseteq(n/4,n/2]\), \(|Q|\ge\eta n\): some legal \(d\le n/4\) has \(\deg_Q(d)\ge c_\eta\log\log(n/t)\) (Turán–Kubilius second moment restricted to \((n/4,n/2]\), truncation \(K=\min(n^{1/8},(n/t)^{1/4})\)) — Attachment F. Its exact limit: \(\int_0^\varepsilon\log\log(1/s)ds\to0\), so the rate is integrable and cannot by itself exclude the capped tail; at \(t\asymp n/H\) it gives only \(\log\log H=o(H)\). An abstract trajectory \(\Delta_t\asymp\log\log(n/t)\), \(|\mathcal M_t|\asymp n/\log\log(n/t)\), \(|Q_t|\asymp n\) is consistent with every accepted sparse-prefix estimate — a proof must use chronology, not more integrable rates.
