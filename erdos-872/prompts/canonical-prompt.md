...
**Codex R34 — R33 ingredients don't compose cleanly; certificate-level anti-concentration needed.** Factor-$\log h$ gap: passing certificates back to targets costs $D_{h,k}/\binom{h}{k} \asymp h$, but desired scored multiplier is only $L \asymp h/\log h$. Standard online set cover literature (Alon et al 2003, Gupta et al 2016) too weak or wrong-model. See researcher-34-codex-resilience-online-promotion.md.

**Pro R35 — static state estimate CLOSED via dyadic expanded windows; only dynamic amortization remains.** Pro A continued past R34's obstruction by using dyadic expanded windows $I_j^* = I_{j-1} \cup I_j \cup I_{j+1}$ (arithmetic locality via $q/p \in (1/2, 2)$ forced by upper-half constraint) rather than short multiplicative cells. The resulting state estimate:
$$|\operatorname{Cl}_h(R, C)| \ll L |F| + \frac{N_h}{\log L}, \qquad L = h/\log h,$$
is RIGOROUSLY PROVEN. Proof: low targets (meeting $\Lambda_L$) handled by fractional charging $|\operatorname{Cl}_h^{\text{low}}| \le O(L) |F|$; high targets (disjoint from $\Lambda_L$) counted via $H_{>L} = H - \log\log L + O(1)$ and $(H_{>L}/H)^h \ll 1/\log L$ in central range. Closes all of: multi-cell product-star entropy, neighboring-cell replacements, lower-defect certificates (via R33 counting lemma), central-rank summation (routine).

**The single remaining gap is dynamic ONLY.** The state estimate needs $|F_{\text{useful}}| \ll N_h/h$ under max-unresolved-degree Shortener strategy $\sigma$. Equivalently: whenever Prolonger scores local boundaries capable of closing $m$ low-window targets, there must be an unresolved-degree vertex of size $\Omega(m/L)$, and $\sigma$'s max-degree claim deletes comparable mass before the same local window is exploited again. If this online amortization holds: $L(n) \ll n/\log\log\log n$, closes $L(n) = o(n)$. Pro R35 saw no counterexample — the simplex-star and multi-cell product-star cascades are both absorbed by the dyadic-local entropy estimate. The only surviving obstruction is dynamic ordering, not static mass. See researcher-35-pro-A-dyadic-state-estimate-closed.md.
...
## R44 additions (2026-04-19)

### R44 Ruled Out

**Attractor pattern — "$\sigma^\star$ + online harmonic domination trivially bound $\mu(F_{\text{useful}})$."** Three independent Pro threads converged on a claimed closure at $\Theta(n(\log\log n)^2/\log n)$ via the same invalid hand-wave: "Shortener's max-unresolved-harmonic-degree play $\sigma^\star$ combined with online harmonic domination $\sum_t \mu(u_t) \le N_h$ automatically controls $\mu(F_{\text{useful}})$." Refutation: online harmonic domination bounds the harmonic mass of *Shortener's* claimed moves $u_t$, not the harmonic mass of *Prolonger's* useful shields $F_{\text{useful}}$. Bounding $\mu(F_{\text{useful}})$ requires an explicit charging/packing lemma from Prolonger's scored set to Shortener's claimed mass — this lemma is not a consequence of the stated tools and is precisely the open problem. Any argument that asserts $\sigma^\star$ controls $\mu(F_{\text{useful}})$ without an explicit packing lemma is this same hand-wave under different language.

Additional pattern errors commonly co-occurring with this attractor: (i) misreading "single-fiber positive-density theorem $\iff L(n) = \Omega(n)$" as "$\Omega(n)$ is refuted" — the equivalence neither proves nor disproves $\Omega(n)$; (ii) reversing refutations into proofs (e.g., "collision-forcing at defect 2 is REFUTED" → "Prolonger succeeds at defect 2 and Shortener is helpless" — actual content: the *proposed Shortener proof* fails, saying nothing about Prolonger's achievable rate); (iii) misreading empirical data $L\log n/n \in [1.55, 1.70]$ on $n \in [10^4, 10^7]$ as ruling out $\Theta(n)$ when $\log\log n$ varies only $2.2 \to 2.8$ at accessible $n$ and the data explicitly cannot distinguish rates.

**SFLU-E first-activation thinning — "strong freshness + local usefulness ⟹ $\sum_D \nu(C(D)) \ll N_h$."** Refuted by explicit local star construction. Fix $C = \{c_1, \ldots, c_{h-1}\}$ and $W = \{w_1, \ldots, w_M\}$ with $M = \lfloor\log h\rfloor$. Defect-two shields $D_i = C \setminus \{c_i\}$ (for $i = 1, \ldots, h-1$) all share the same defect-1 coface $C$, all satisfy the density-increment hypothesis $\mu(\mathrm{Sh}^{\text{loc}}_h(C)) \ge c\theta \mu(\mathrm{Sh}^{\text{loc}}_h(D_i))$ at $\theta \asymp 1/\log h$, and all are strongly fresh (no prior Shortener or Prolonger sterilization) before Prolonger activates. Union lift $\nu_{h,2}^\cup(\{D_i\}) \asymp M^2/h$ vs single-coface lift $\nu_{h,1}(C) \asymp M/h$: gap factor $\asymp h$. Strong freshness alone does not prevent Prolonger from activating many exceptional shields sharing the same coface-lift before $\sigma^\star$ resolves it. The packing step is a genuinely new assumption, not a consequence of freshness + locality.

**R35 residual floor as structural block to the sharp conjectured rate.** The state inequality
$$\mu(\operatorname{Cl}_h) \ll \frac{h}{\log h} \mu(F_{\text{useful}}) + \frac{N_h}{\log(h/\log h)}$$
has residual term $\asymp N_h/\log h$. Even in the idealized case $\mu(F_{\text{useful}}) = 0$, summing over central ranks $h \sim \log\log n$ gives only $L(n) \ll n/\log\log\log n$. Since $(\log\log n)^2/\log n = o(1/\log\log\log n)$, the R35 residual alone forbids reaching the conjectured sharp rate $n(\log\log n)^2/\log n$. Consequently, any sharp-rate closure via the R35 framework is impossible; a replacement state inequality with smaller central-rank residual is required, OR the conjectured sharp rate is wrong.

### R44 Established (new conditional result)

**SFLU-E conditional theorem.** Define the lifted harmonic shadow measure $\nu_{h,k}(D) = \mu_h(\mathrm{Sh}^{\text{loc}}_h(D))/\binom{h}{k}$ for defect-$k$ lower face $D$ with local shadow in the dyadic expanded window $I_j^\star$ (replacement ratio $q/p \in (1/2, 2)$). Say $D$ is **strongly fresh** if its local shadow has not been touched by any prior Shortener claim, prior Prolonger move, or prior Prolonger-scored shield (crucially, the stricter requirement than weak freshness excludes Prolonger-caused pre-sterilization, which is what the common-core refutation exploits). Say $D$ is **locally useful** if its local shadow is nonempty in the unresolved rank-$h$ board, admits a Shortener-destroyer, and is not already dense-packet-absorbed. For $s(k) = \min\{k-1, \lfloor h/\log h \rfloor\}$, define $\mathcal E_{k,\theta}^{\text{sf,loc}}$ as the family of defect-$k$ strongly-fresh locally-useful exceptional shields with a coface certificate $C(D) = D \cup Q(D)$, $|Q(D)| = s(k)$, satisfying $\mu_h(\mathrm{Sh}^{\text{loc}}_h(C(D))) \ge c_0 \theta \mu_h(\mathrm{Sh}^{\text{loc}}_h(D))$. Then, **assuming the certificate-packing estimate (CP)** $\sum_{D \in \mathcal E_{k,\theta}^{\text{sf,loc}}} \nu(C(D)) \ll N_h$:
$$\nu_{h,k}^\Sigma(\mathcal E_{k,\theta}^{\text{sf,loc}}) \ll_C \theta^{-1}(Ck/h)^{s(k)} N_h.$$
The individual comparison step (binomial ratio + density increment) is rigorous; only the family-level step is conditional on (CP). See `researcher-44-pro-1-followup-sflu-e-estimate.md` and the self-refutation `researcher-44-pro-1-followup-2-formal-concession.md`.

### R44 Open Question — three precisely stated lemmas

**(A) Certificate-packing theorem.** Under strong freshness + local usefulness:
$$\sum_{D \in \mathcal E_{k,\theta}^{\text{sf,loc}}} \nu(C(D)) \ll N_h.$$
Must handle simultaneous activation and shared cofaces. Candidate approaches: a Carleson-style packing bound in the lifted harmonic measure; a structural theorem showing high-multiplicity shared-coface stars are already absorbed by the dense-packet projection; a dynamic injective-charging lemma under $\sigma^\star$ that assigns each exceptional shield to a distinct piece of newly-killed Shortener mass.

**(B) Sharper defect-two estimate.** $\mu(\mathcal E_{2, 1/\log h}^{\text{sf,loc}}) \ll N_h/h$ (rather than the $(\log h/h) N_h$ that conditional SFLU-E gives at $k=2$, $s=1$). The $k=2$ case currently loses a factor $\log h$ against the $L = h/\log h$ multiplier in R35. Requires a sharper dense-packet projection specifically at $k=2$, or an argument that the defect-two residue is absorbed elsewhere before entering $F_{\text{useful}}$.

**(C) Replacement state inequality** $\mu(\operatorname{Cl}_h) \ll L \mu(F_{\text{useful}}) + R(h)$ with residual $R(h) \ll N_h \cdot (\log\log n)^2/\log n$ at central rank $h \sim \log\log n$. This would reach the conjectured sharp rate if (A) and (B) also hold. Alternative framing: prove that no such replacement exists, refuting the conjectured $\Theta(n(\log\log n)^2/\log n)$ rate and establishing the true sharp rate as $n/\log\log\log n$ via R35 alone.

Implications:
- (A) + (B) ⟹ $\mu(F_{\text{useful}}) \ll N_h/h$ ⟹ $L(n) \ll n/\log\log\log n$ via R35 ⟹ closes $L(n) = o(n)$.
- (A) + (B) + (C) ⟹ $L(n) \ll n(\log\log n)^2/\log n$ ⟹ closes the conjectured sharp rate.
- Refutation of (C) combined with (A) + (B) ⟹ sharp rate is $n/\log\log\log n$, conjectured rate is wrong.

## R46 additions (2026-04-19)

### R46 Established

**$\sigma^\star$ smallest-legal-prime lemma (rigorous, game-theoretic).** Under the max-unresolved-harmonic-degree Shortener strategy $\sigma^\star$, at every turn Shortener claims the smallest remaining legal prime. Proof: for legal primes $p < q$, replacing $q$ by $p$ in any rank-$h$ squarefree facet gives an unresolved facet of no larger product and strictly larger harmonic weight $1/p > 1/q$, so $\deg_t(p) > \deg_t(q)$. Any legal composite $R$ has $\deg_t(R) \le \deg_t(r)$ for any prime factor $r \mid R$, since rank-$h$ multiples of $R$ are a subset of rank-$h$ multiples of $r$. Therefore some legal prime attains the max, and among primes the smallest wins. Three independent Pros derived this in R46; sandbox-verified.

### R46 Ruled Out

**ST-capture dynamic lemma (Angle 2, time-resolved Carleson packing).** Refuted by burn-in + forced-leaf construction, convergent across three independent Pros. Construction: prime packet $I = [P, 2P]$ with at least $h^2 + h$ primes, core $C = \{c_1, \ldots, c_{h-1}\} \subset I$, leaf set $W = \{w_1, \ldots, w_M\} \subset I$ with $M = h^2$. Defect-2 shield $D = C \setminus \{c_1\}$, coface $C(D) = C$, local shadow $S(D) = \{C^\times q : q \in W\}$. Prolonger's burn-in moves are fresh primes $R_p > n/2$ outside $I$; $\sigma^\star$ is forced by the smallest-legal-prime lemma to claim every prime $p$ below $I$ in increasing order, none touching $S(D)$. After Prolonger activates with $F_0 = C^\times \cdot w_1$, all cores $c_i$ and $w_1$ become illegal divisors of $F_0$. $\sigma^\star$ is then forced to claim the smallest remaining legal prime — a leaf $w_2 \in W$ — which touches only one facet in $S(D)$. Capture ratio $\mu_h(S(D) \cap U_{\tau(D)}) / \nu(C(D)) \le 2h/M = 2/h \to 0$. Violates any absolute constant $c > 0$ in ST-capture. The proposed "legal-leaf balance bound" salvage $p_{\min}^{\text{legal}}(C) \cdot \sum_{q \in W(C)} 1/q \ll \binom{h}{r(D)}$ is false by the same construction.

**Game-legality attractor: "$\sigma^\star$ claims prime divisors of Prolonger's played elements."** In the antichain divisibility game, once Prolonger plays $F$, no prime factor $p \mid F$ can be legally claimed by Shortener (the resulting set $\{\ldots, F, \ldots, p\}$ would violate the antichain rule: $p \mid F$). $\sigma^\star$ is a strategy that picks the max-unresolved-harmonic-degree LEGAL claim — illegal moves are not in the selection set. Any argument that has Shortener "claim $c_1$" or "claim $x_1$" where $c_1, x_1 \in F$ and $F$ is already played is game-theoretically invalid. Two Pros independently made this error attempting to refute Angle 1 in R46. Correct template (from R46 Pros #1/#2/#3): force the smallest legal prime to be a specific non-divisor of the activator via pre-activator preparation (burn-in exhausts smaller primes; activator uses only $w_1 \in W$, leaving $z$ or $w_2$ as smallest legal and non-dividing).
