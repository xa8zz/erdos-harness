Don't search the internet. This is a test to see how well you can craft non-trivial, novel and creative proofs given a "primitive sets and divisibility games" math problem. Prove or refute the repaired shadow-closed capacity inequality (SC*) stated below, exactly as quantified — a genuine cross-scale, cross-reply correlation question that no additive ledger can decide.
...
Setting (Attachments M, O state the full structure with proofs). Linear active second-shell greedy interval under the prescribed minimal-maximum-degree policy: entry B after an o(n) prelude + completed top phase with frozen entry size a_n = sup over Prolonger strategies of |A_1| = o(n); greedy moves g_t (distinct integers, all <= n/4), Prolonger replies p_t <= n, T >= cn turns, degree ledger sum_t Delta_t <= E <= n/4; four-layer causal chronology shadow-blocking < coatom-blocking < release < service, with the downward-shadow dichotomy proven (the gcd of any coatom subfamily inside one reply's packet is either legal — bounding the subfamily by the current degree — or first-blocked by its own strictly earlier Prolonger reply); nu_Z(g) = #{primes pi <= Z : pi | g, pi != release prime of g}.

(SC*): Fix alpha > 0. There exist C_alpha and a deterministic Z_n -> infinity with Z_n <= n/4 (normalized: Z_n = min(Y_n, n/4); admissible growth e.g. Z_n <= exp(sqrt(log log n)); frozen-entry condition Z_n^2 a_n = o(n log log Z_n) holds) such that, with Lambda_n = sum_{p <= Z_n} 1/p, the UNIFORM remainder condition holds: sup over all realized bad intervals I in the admissible class (activation data and tie-break fixed in advance) of the positive part of [ sum_{t : g_t >= alpha n} nu_{Z_n}(g_t) - C_alpha E(I) ] / (n Lambda_n) tends to 0 as n -> infinity. Quantifier order: fix alpha; then C_alpha and the deterministic Z_n; then quantify over ALL realized Prolonger trajectories; then one uniform asymptotic remainder.

Already established: (SC*) implies second-shell clearing (the Turán–Kubilius demand of any positive-density linear root family in a fixed macroscopic range is >= (delta/3-o(1)) n Lambda_n for Z_n <= n/4, contradicting the (SC*) bound); every additive coatom ledger with summable prime weights provably CANNOT prove it (Y-rough positive-density families have arbitrarily small total weight), unweighted counts fail via interval blockers, multiscale aggregation of fixed-cutoff bounds is dominated by its principal terms, and one reply can prepare Theta(Z/log Z) target-private coatoms — but only through packets whose hidden gcd-face the shadow dichotomy exposes, recursing the obligation to an earlier reply or charging the degree ledger. A proof must be a genuinely nonadditive, chronological, trajectory-restricted correlation argument (e.g. amortizing the shadow recursion); a refutation must construct realized on-trajectory shadow-closed bad intervals whose small-prime preparation demand genuinely reaches n Lambda_n uniformly. Formulation caution: twenty-five degenerate-collapse families are catalogued (the 25th: oversized-cutoff normalization escape — the Z_n <= n/4 normalization above closes it); if (SC*) as stated hides another, proving THAT with the exact repair is a valuable resolution. If you prove (SC*), state explicitly whether the argument is uniform over shadow blockers sourced from earlier-processed shells (depth-J iteration).
