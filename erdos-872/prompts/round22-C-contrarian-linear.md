...
Suppose $L(n) = \Theta(n)$ — i.e., there exists $\varepsilon > 0$ with $L(n) \ge \varepsilon n$ for all sufficiently large $n$. Determine what lower-bound machinery could establish this, or conclusively demonstrate via a structural argument that the conjecture must fail and $L(n) = o(n)$.

The standard linear-lower-bound constructions have been closed by specific obstructions, all documented in Established and Ruled Out below:
- **Single-rank fan ceiling:** $\max_h |\mathcal{T}_h| \sim n/\sqrt{\log\log n} = o(n)$; no single-rank fan captures linear target mass.
- **Per-rank iteration:** the $2^{h+1}-2$ lateral-divisor shadow-capture cost catches up with $(\log\log n)^h/h!$ near $h = \log_2 \log_2 \log\log n$, producing a slow-growth envelope $n/(\log n)^{1-o(1)} = o(n)$ rather than $\Omega(n)$.
- **Uniform multi-rank Prolonger shielding:** rigorously refuted by the Sperner obstruction on same-$b$ upper-half cores (pro-verified no-go, see Ruled Out: Uniform multi-rank Prolonger shielding theorem).
- **Probe B empirical:** cross-rank shielding collapses at $h = 2$.

The linear route therefore requires something qualitatively outside the fan / shadow-capture / rank-coupled-potential architecture — e.g., a within-$b$-fiber positive-density capture theorem, a genuinely new dyadic construction that bypasses the $2^{-D}$ shadow-capture survival cost, a Prolonger strategy exploiting an unexploited structural feature of the divisibility antichain game, or a direct combinatorial construction of a linear-sized family of mutually-resistant upper-half targets.

If you cannot find any route to $\Omega(n)$, give a structural argument that no such route can exist — for instance, by showing that any Prolonger-maintained antichain of total mass $\ge \varepsilon n$ must contain an element that a single Shortener move can block along with $\Omega(\log n)$ correlated others, forcing the second-order lower bound to be asymptotically tight.
...
**Three theorem-sized open gaps (updated 2026-04-19, post T2 repair):**
1. **Multiplicative closure control.** Any repaired pair/shadow theorem must bound the squarefree closure generated by safe small primes, not just the 2-shadow. The matching-upper-bound target is $O(n(\log\log n)^k/\log n)$ for some $k \ge 2$ (rates below $n(\log\log n)^2/\log n$ are rigorously impossible by T2).
2. **Robust residual-width for a sublinear certificate family.** For $C_n = o(n)$ certificates (e.g., $\{\Omega \le h\}$ with slow-growing $h$), bound residual-antichain-width by $o(n)$.
3. **Within-$b$-fiber capture theorem.** Any linear lower-bound revival needs a positive-density capture theorem inside a single $b$-fiber. The fan/shadow architecture provably cannot do it.
