...
## The angle — Sparse matching-residue packing (SMRP) trichotomy

For high-multiplicity shared-coface defect-two clusters $\mathcal F = \{D_i\}$ (strongly fresh, locally useful), prove the trichotomy: either
$$\nu_{h,2}^\cup(\mathcal F) \ll \frac{N_h}{h},$$
or $\mathcal F$ admits a genuine dense defect-three completion (absorbed by dense-packet projection), or the sparse matching-residue portion of $\mathcal F$ satisfies a packing estimate
$$\sum_{\text{sparse matching-residue clusters}} \nu_{h,2}^\cup(\mathcal F) \ll \frac{N_h}{h}.$$

Sharp version (for the conjectured rate): the sum is $\ll N_h \cdot H^2 \log h \cdot e^{-h}/\log n$.

**Motivating obstruction.** The matching-star construction (prior round) shows: with common coface $C$, $|I| \asymp h$ arms $D_i = C \setminus \{c_i\}$, private matchings $M_i$ on disjoint supports $|V_i| \asymp m^2$ with $m = \lfloor \log h \rfloor$, and $J \asymp m^2$ edges per matching, one gets $\nu_{h,2}^\cup \asymp m^2/h \cdot N_h$ — larger than $N_h/h$ by factor $(\log h)^2$. Every defect-three subface has density $\le m^{-4} = o(\theta)$, so rank-three completion dichotomy fails. The matching-star evades both the union-lift bound and rank-three absorption. Either the third alternative (SMRP) holds, or a fourth alternative exists.

**Formal parameters** (make explicit in any proof): for cluster $\mathcal F$ with common coface $C$, arm residue graph $G_i$ has vertex mass $\sigma_i = \sum_{v \in \mathrm{supp}(G_i)} \omega_v$ and edge mass $B_i = \sum_{\{u,v\} \in G_i} \omega_u \omega_v$. The defect-two density increment gives $B_i \asymp \theta^{-1} A$ where $A = \mu(\mathrm{Sh}(C))$, but places no upper bound on $\sigma_i$. The matching-star has $\sigma_i \asymp B_i$.

**Specific sub-questions either direction must address:**

*For a proof of SMRP trichotomy:*
- Give a precise definition of "sparse matching-residue cluster" (e.g., $\sigma_i^3 \gg \theta^{-1} B_i$ and $\Delta_w(G_i) := \max_v \sum_{u: \{u,v\} \in G_i} \omega_u \omega_v \ll B_i$).
- Prove the packing $\sum \nu_{h,2}^\cup \ll N_h/h$ for sparse matching-residue clusters. Handle game-theoretic dynamics under $\sigma^\star$: the sparse residues must be charge-able to Shortener's legal moves.
- Extend to the sharp rate bound $N_h H^2 \log h \cdot e^{-h}/\log n$.

*For a refutation (i.e., a fourth alternative bypassing all three):*
- Construct a strongly fresh locally useful cluster that avoids all three alternatives: large union mass, no dense rank-three completion, not "sparse matching-residue" under any reasonable definition.
- All constructions must be game-legal: each Prolonger score must be an actual legal move respecting prior moves; each $\sigma^\star$ response must not divide any previously-played element (smallest-legal-prime lemma).
