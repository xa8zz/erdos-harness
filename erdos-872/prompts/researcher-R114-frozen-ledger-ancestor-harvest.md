Don't search the internet. This is a test to see how well you can craft non-trivial, novel and creative proofs given a "primitive sets and divisibility games" math problem. Provide a full unconditional proof or disproof of the single statement (AH*) defined in the Problem section.

## Problem

For \(n\ge2\), two players alternately choose previously unchosen integers from \(\{2,\ldots,n\}\), always keeping the chosen set an antichain under divisibility (a primitive set). The game ends when the set is maximal. Prolonger moves first and maximizes the total number of moves; Shortener minimizes it; \(L(n)\) is the length under optimal play. Prove or disprove the frozen-ledger ancestor-harvest statement (AH*), defined as follows using the framework of the two attached analyses. There is a fully specified legal online Shortener policy and a prescribed post-trapping activation time \(s_X\) for each dyadic shell — fixed in advance of, and independent of, all future play — at which the shell's reservoir \(Q_X^0:=Q_X(s_X)\), its full canonical latent-face family \(\mathscr F_X^0\), and the face weights \(w_X^0(F)\) are frozen, with no later reinitialization or rebasing of blocks permitted and processing beginning immediately, such that uniformly at every stopping time \(t\) before the degree cap or the halving of the frozen reservoir, the cumulative frozen weight of faces lost through first-exit direct ancestors satisfies \(\operatorname{Anc}_X(t)\le o(X)+O(Ht)\) against every Prolonger strategy.

The first attached analysis proves this frozen form is exactly what the conditional pipeline (AH) => L(n)=o(n) needs (its terminal-delay and cost-only degeneracies are closed by the freezing). The second attached analysis closes the residual channel and the trace-1 anchor and shows why max-degree potentials and lifetime divisor-count bounds cannot prove the inequality — the direct-ancestor channel erases whole latent families at contemporaneous area cost \(O(H)\). Formulation caution: if (AH*) as stated still admits a degenerate witness or collapses to the full problem, proving THAT is a valuable resolution; state it explicitly. Resolve (AH*) in either direction with a complete proof.
...
## Attached analysis 1 (verbatim): the degeneracy analysis defining the frozen-ledger requirements (its Section 5 is the specification adopted above)
...
## Attached analysis 2 (verbatim): the reduction isolating the ancestor-harvest inequality and closing the residual and anchor channels (prime-product lemma requires S nonempty)
