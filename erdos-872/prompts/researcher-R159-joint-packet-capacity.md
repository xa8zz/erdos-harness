Don't search the internet. This is a test to see how well you can craft non-trivial, novel and creative proofs given a "primitive sets and divisibility games" math problem. Resolve the joint packet-capacity question stated below: prove the capacity bound that excludes a linear bi-causal reply system, or construct a Prolonger strategy realizing one on-trajectory.
...
The target. Attachment K proves that in any linear active second-shell greedy interval (entry position B reached by an o(n) prelude + completed top phase, greedy maximum-deletion Shortener moves g_t, Prolonger replies p_t, T >= cn turns, final survivors X, E = |Q_1(B)\X|), the following are all forced and are settled inputs:
- every greedy root g_t is either minimal at entry (only o(n) of these, by static completion) or has a UNIQUE release index gamma(t) < t: it first became minimal immediately after reply p_{gamma(t)};
- the exact prefix Hall equality: for EVERY prefix s <= T, s = |{t<=s : g_t minimal at entry}| + sum_{j<s} |C_j(s)| where C_j(s) = {t<=s : gamma(t)=j}; hence sum_{j<s}|C_j(s)| >= s - o(n) for every linear prefix (endpoint-only accounting is the 21st catalogued collapse, postponed-service laundering);
- every final-survivor divisor deadline is Prolonger's: k_X(d) > E/s forces d | p_j for some j < s (causal orientation; Shortener's greedy moves are provably incomparable with survivor-incident divisors);
- joint packet containment: for each reply p_j, the lcm of its blocked divisors B_j together with the last legal ancestors lambda_j(r) of every root r it releases (r in C_j) must divide p_j, and p_j <= n.

The open object — the JOINT PACKET-CAPACITY question: bound the cumulative service a single reply sequence can provide. Each reply p_j <= n must pack, inside one integer, (i) the divisors whose survivor deadlines it discharges and (ii) the last-ancestors of every root it releases, while every released root feeds a later greedy turn whose deletions sum to at most E (the degree ledger sum_t Delta_t <= E <= n/4). Prove that no reply sequence can sustain sum_{j<s}|C_j(s)| >= s - o(n) for all linear prefixes simultaneously with the survivor deadlines and containments (this proves FSC(2,eta) for the prescribed minimal-maximum-degree policy and tie-break; state whether it iterates to deeper shells, where replies from earlier shells provide extra upward channels), or construct an on-trajectory Prolonger strategy realizing the full bi-causal system (this closes the weakest sufficient route; state exactly what it shows about other policies and L(n)). Formulation caution: twenty-one degenerate-collapse families are catalogued; if the question as posed hides another, proving THAT with the exact repair is a valuable resolution. Known counter-pressures a proof must overcome: one common-core reply can release k ~ log n/log log n roots at once (Attachment J), and one reply divisible by many survivor-incident divisors can discharge many deadlines at once; known pressure a construction must survive: future-service pressure Delta_t >= (|X|+T-t+1)/(n/8-t+O(1)+o(n)) couples late-interval degrees to the shrinking primitive room below n/4 (Attachment K, Corollary 2).
