---
id: R178-scheduling-canonicalization
type: synthesis
date: 2026-07-24
intent: >
  Canonicalize two scheduling clauses of the manuscript whose literal reading
  was under-specified or not a legal event of the formal game, surfaced
  independently by a human line-audit and by a Lean 4 formalization of the
  game semantics; no mathematical content changes.
predecessors: [R177-repaired-manuscript, V177-fable-diff-audit]
action:
  kind: supersedes
  target: R177-repaired-manuscript
failure_mechanism: >
  Two scheduling defects in section 4 of the superseded text: (i) the
  four-clause Shortener response rule was silent when Prolonger's selection
  terminates the active cone's local game — canonicalized to
  first-rule-that-supplies-a-legal-move precedence; (ii) local-trace step 4
  inserted a projected Prolonger pass before Shortener's first local
  selection, which is not a legal robust-game event because the opening-end
  declaration schedules Shortener directly — the insertion is now restricted
  to after the first local Shortener selection. No constant, bound, or
  inequality changed.
claim: >
  Same theorem, L(n) = o(n), by the same argument; the Shortener response
  rule and the projected local-trace construction are now stated in
  directly formalizable form.
verifiers_at_time:
  - [fable-V177-diff-audit, agrees]
  - [codex-lean-tier0-formalization, agrees]
...
Shortener applies the **first** of the following rules that supplies a legal move.

If Prolonger has just selected a vertex in an already active cone, Shortener responds there according to an optimal local robust strategy, after division by its tag.

Otherwise, Shortener performs a **root sweep** whenever one is available:
...
If no root sweep is available, Shortener advances an active local game whose local strategy supplies a legal move, treating a global Prolonger selection or pass outside that cone as a local pass when necessary. If several active local games supply such a move, she uses any fixed deterministic choice among them.

If no root sweep is available and no active local strategy supplies a legal move, Shortener selects an arbitrary legal vertex outside all root cones. This final rule is applicable whenever the global position is nonterminal and no preceding rule supplies a legal move: a legal vertex in an unprocessed cone would have a live root by §4.1, a legal vertex in an active cone would leave its local game nonterminal, and a cleared cone has no live vertex.
...
- If \(C_t\) received \(k\) selections during the global opening, then \(k\le b\), and at most one further local Prolonger selection can occur before the first local Shortener response.
- If \(C_t\) received no opening selection and is first activated later, then at most two Prolonger selections can occur in it before the first local Shortener response: the activation selection and one return selection.
...
4. If Shortener selects inside \(C_t\) after a global Prolonger selection or pass outside \(C_t\), insert a local Prolonger pass before that Shortener selection. This insertion applies only after Shortener's first local selection; before that first selection no pass is inserted, because the end of the local opening already schedules Shortener directly.
5. Before the first local Shortener selection, regard all projected Prolonger selections as the opening block.

After Shortener’s first local selection, every actual Prolonger selection inside an already active cone receives an immediate Shortener response there. Thus no two later local Prolonger selections occur without an intervening local Shortener selection. The resulting local history is a legal robust game with at most \(b+1\) opening selections.

The optimal local strategies can be run simultaneously. There is never more than one active cone requiring an immediate local response after a global Prolonger selection: it is the cone in which that selection was made. Every other local game remains frozen. When Prolonger acts outside all active cones, Shortener may sweep a root, advance one active local game while treating the external action as a local pass, or use the exceptional outside-vertex rule from §4. External erasure histories may be correlated across cones, but each local strategy handles every adaptive quotient-upset erasure history; no independence is required.
...
Every selected vertex belongs to exactly one of the following three classes:

1. a vertex in an active cone;
2. a root tag fired by Shortener;
3. a vertex outside all root cones.

These classes are exhaustive and disjoint. Every Prolonger selection in a root cone makes that cone active, and all later selections in that cone belong to class 1. A root fired by Shortener belongs to a cone that was not active; that selection clears the cone, so it can never subsequently become active. Every remaining selection is outside the union of the root cones and belongs to class 3.
