---
id: R171-ordered-sweep-vaccination-impasse
type: research
date: 2026-07-16
intent: >
  Full solution attempt (unconditional proof or disproof of L(n) = o(n) for
  the divisor-antichain game) from the full compiled state; the response
  (a 103-minute reasoning run) was left unharvested until 2026-07-24, when it
  was retrieved as the pre-publication collision check.
predecessors: []
action:
  kind: standalone
claim: >
  No unconditional proof or disproof. Two pieces are established and one
  impasse is isolated. (1) Ordered-sweep lemma: if Shortener sweeps any
  candidate family F within [2, n/2] in a linear extension of divisibility
  (skip illegal, play first legal), then any divisor d in F of a surviving
  target divides some Prolonger move — Shortener self-blocking is
  impossible. (2) Universal top-half vaccination lemma: for every F within
  [2, n/2] the set V(F) = {d*floor(n/d)} is a primitive subset of (n/2, n]
  of size at most |F| such that every d in F divides a member of V(F) while
  the rest of the top half stays legal; hence every PREDECLARED o(n)-sized
  family of Shortener moves has a static o(n)-cost defeater and the swept
  down-shadow certificate has no density consequence. The impasse: the
  vaccination position is not known to be reachable against an adaptive
  Shortener, and the sweep certificate is not known to be closable without
  predeclaration — recovering a proof requires an online
  reachability/anti-reachability theorem.
implications:
  - Any winning Shortener strategy must be adaptive (online) rather than a
    predeclared move family; a static family is vaccinated at cost o(n).
confidence_at_time: medium
strategy_dependence: independent
prompt: erdos-872/prompts/researcher-R171-full-compilation-freeroam.md
---
